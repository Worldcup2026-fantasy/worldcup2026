      ...
      {view === "list" && (
        <>
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">My Leagues</h1>
              <p className="text-sm text-gray-400 mt-1">Compete privately with friends</p>
            </div>
            <button
              onClick={() => setShowModal(true)}
              className="bg-blue-600 text-white text-sm font-medium px-4 py-2 rounded-xl hover:bg-blue-700 transition-colors flex items-center gap-2"
            >
          ...
          {loadingLeagues ? (
            ...
            <div className="text-center py-16">
              ...
              <h2 className="text-lg font-medium text-gray-900 mb-2">No leagues yet</h2>
              <p className="text-sm text-gray-400 mb-6">Create a private league and challenge your friends.</p>
              <button
                onClick={() => setShowModal(true)}
      ...
      {view === "detail" && activeLeague && (
        <>
          <div className="flex items-center gap-3 mb-6">
            <button onClick={goBack} className="text-gray-400 hover:text-gray-700 text-sm flex items-center gap-1">
              ← Back
            </button>
            <div className="h-4 w-px bg-gray-200" />
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-semibold text-gray-900 truncate">{activeLeague.name}</h1>
              <p className="text-xs text-gray-400">
            ...
            <button
              ...
              className="text-xs text-gray-400 hover:text-red-500 transition-colors px-3 py-1.5 rounded-lg border border-gray-200 hover:border-red-200 flex-shrink-0 disabled:opacity-50"
            >
          ...
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-sm font-medium text-gray-700">Standings</h2>
            <span className="text-xs text-gray-400">GW3 · Live</span>
          </div>
          ...
          {loadingStandings ? (
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-8 text-center">
              ...
              <p className="font-medium text-gray-900 mb-1">Just you so far</p>
              <p className="text-sm text-gray-400">Share the code above to invite friends.</p>
            </div>
          ) : (
            <LeagueStandingsTable members={standings} currentUserId={userId ?? undefined} />
          )}

          <p className="text-xs text-center text-gray-400 mt-4 bg-gray-50 rounded-xl py-3 px-4">
